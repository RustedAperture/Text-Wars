...
Date: March 21, 2014
Filename: Game.py
Description: Text based war game using python, rewrite of my original text wars.
Version: 1.0.0
versions since relese: 1
'''
...
hasplayed = str("no")
money = 500
troops = [1, 0] # troops, extra troops
enemy_troops = 0
buy_troops = 0
battles_won = 0
total_battles = 0
...
def saveload(function, filename):
	...
	if function == "load":
		with open(filename, 'rb') as f:
			money, troops, discount, token, HP, powerups, battles_won, total_battles = pickle.load(f)
	elif function == "save":
		with open(filename, 'wb') as f:
			pickle.dump([money, troops, discount, token, HP, powerups, battles_won, total_battles], f)
	else:
		print ("Invalid function recieved for def:savegame")
...
def wait_clear(mode, n='', mode1=''):
	if mode == "wait":
		...
		print ("Incorrect function recieved for def:wait_clear")

# Star game or restart game 
def startgame():
	print ("Starting Game")
	for i in range(50+1):
		time.sleep(0.1)
...
def intro():
	...
	print ("Text Wars\nversion:", version, "\n")
	print ("changelog is viewable on the github\n")
	print ("Please enter your name: ", end="")
	username = str(input())
	if username == "":
		print ("Invalid name or no name has been entered")
		intro()
	else:
		wait_clear("clear")
		print ("Welcome Commander", username)
		try:
			saveload("load", username)
			hasplayed = str("yes")
		except:
			print ("\nA game save is being created for you.")
			saveload("save", username)
			print ("\nlet's get started with how the game works\n")
...
def play_b4():
	...
	if hasplayed == "yes":
		print ("A game was loaded for you.\nWould you like to continue or start a new game. (new/old): ", end="")
		newgame = str(input())
		if newgame == "new":
			print ("Are you sure you remember how to play? (yes/no): ", end="")
			confirm = input()
			if confirm == "yes":
				money = 500
				troops = [1, 0]
				...
				print("invalid response for function def:play_b4:hasplayed:yes:confirm")
				play_b4()
		elif newgame == "old":
			print ("Are you sure you remember how to play? (yes/no): ", end="")
			confirm = input()
			...
			print ("Invalid response for function def:play_b4:hasplayed:yes")
			play_b4()
	elif hasplayed == "no":
		wait_clear("clear")
		training()
	else:
		print ("Invalid Response for function def:play_b4:hasplayed")
...
def menu(mode):
	...
	if m_location == '1':
		...
		print ("Invalid response on function def:menu")
		wait_clear("wait", n=2, mode1="clear")
		menu(mode)

# Move the troops to the correct location.
def transport(mode):
	
	global troops
	
	if troops[0] < int(10) and troops[1] > int(0):
		troops[0] += 1
		troops[1] -= 1
...
def info(mode):
	...
	print ("-----------------------------------------")
	print ("| Troops:         ", str(troops[0]).rjust(20), "|")
	print ("| Extra Troops:   ", str(troops[1]).rjust(20), "|")
...
def store(mode):
	...
	if m_location == '1': # Troops
		...
		try:
			...
			store(mode)
		if buy_troops < 0:
			print ("I'm sorry you cannot sell humans.")
			wait_clear("wait", n=2, mode1="clear")
			store(mode)
		if discount >= 1:
			if buy_troops > discount:
				...
				print ("New amount of troops: ", troops[0])
...
# Training sequence
def training():
	print ("Welcome to boot camp sir. When you are ready to play hit retire on the main menu.")
	cont = input("Press enter to continue.")
	menu(1)
...
def battle(mode):
	...
	if troops[0] == 0:
		...
		saveload("save", username)
		menu(mode)
	elif enemy_troops > troops[0] and enemy_troops >= 1:
		print ("Sir, they attacked before we had the chance.")
		print ("We lost a member of our family today")
		HP -= 1
		troops[0] -= 1
		print ("we now have: ", troops[0], " troops.")
		print ("HP: ", HP)
		total_battles += 1
		wait_clear("wait", n=7, mode1="clear")
		transport(mode)
